...
def display_kpi_row(snapshot: dict, cs: str = "$") -> None:
    ...
    with col1:
        st.metric(
            "💰 Capital investi",
            f"{cs}{snapshot.get('invested_total_eur', 0):,.2f}",
        )
    ...
    with col3:
        st.metric(
            "📈 Valeur actuelle",
            f"{cs}{snapshot.get('market_value_eur', 0):,.2f}",
        )
...
def display_price_info(snapshot: dict, cs: str = "$") -> None:
    """Affiche prix moyen, prix marché et frais cumulés."""
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Prix moyen d'achat", f"{cs}{snapshot.get('avg_buy_price', 0):,.2f}")
    with col2:
        st.metric("Prix marché actuel", f"{cs}{snapshot.get('market_price', 0):,.2f}")
    with col3:
    ...
    with col4:
        ...
        st.metric(f"💸 Frais ({cs})", f"{cs}{fee_usd:,.4f}")
